...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog="O iOS que manda se anima. Aqui a gente reconstitui a receita que já funcionou no aparelho: HEVC hvc1, 1080x1920, ~1s, 60 fps. h264, 3s e resolução nativa do iPhone 16 Pro Max (1320x2868) quebram.",
    ...
    parser.add_argument("--check", nargs="+", metavar="ARQUIVO", help="confere MOV (e HEIC) na receita")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.check:
            ...
            print(assert_recipe(movie, still))
            return 0
        if not args.input:
            _parser().print_help()
            return 2
        ...
        print(f"  HEIC {outputs['heic']}  (JPEG/JFIF com MakerApple 17; o par que já animou era assim)")
        if "pvt" in outputs:
